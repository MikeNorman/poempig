#!/usr/bin/env python3
"""
Script to identify and populate missing titles and authors for well-known poems/quotes.
Uses GPT to identify pieces with high confidence.
"""
...
def identify_poem(text, title="", author=""):
    """
    Use GPT to identify a poem/quote and return title, author with confidence.
    ...
    try:
        ...
        prompt = f"""
        Identify this poem or quote. Return ONLY a JSON object with this exact structure:
        {{
            "title": "exact title or null if unknown",
            "author": "exact author name or null if unknown", 
        ...
        Rules:
        - Only return "high" confidence if you're 100% certain
...
def main():
    """Identify and update missing titles/authors for items."""
    ...
    print("🔍 Starting poem/quote identification process...")
    ...
    for item in items_result.data:
        try:
            ...
            # Identify the piece
            identification = identify_poem(text, current_title, current_author)
            ...
            if identification['confidence'] in ['high', 'medium']:
                update_data = {}
                ...
                print(f"   ⏭️  Confidence too low ({identification['confidence']})")
